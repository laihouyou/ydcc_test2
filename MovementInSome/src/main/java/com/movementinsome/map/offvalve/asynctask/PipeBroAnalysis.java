...
public class PipeBroAnalysis extends AsyncTask<String, Void, String> {
	...
	public PipeBroAnalysis(PipeBroAnalysisTools pipeBroAnalysisTools,
			boolean isAnalysis, Point point) {
		// TODO Auto-generated constructor stub
		this.pipeBroAnalysisTools = pipeBroAnalysisTools;
		this.context = pipeBroAnalysisTools.getContext();
		this.isAnalysis = isAnalysis;
		this.map = pipeBroAnalysisTools.getMap();
		this.point = point;
		gson = new Gson();
	...
	protected String doInBackground(String... params) {
		...
		String target =  pipeBroAnalysisTools.getPipeBreakenData();
		JSONObject jo=null;
		try {
		...
		if (params[0].equals("ANALYSIS")) {
			...
			if (null == valuePair.get(String.valueOf("geometricnetwork")))
				valuePair.put("geometricnetwork", "SDE.WATER_NETWORK_Net");
			valuePair.put("f", "pjson");
		...
		if (result == null){
			...
			pipeBroAnalysisTools.setResult(result);
		}
	...
	protected void onPostExecute(String result) {
		...
		if (result.equals("SUCCEED") || result.equals("SHOW")) {
			...
			pipeBroAnalysisTools.setData(data);
			pipeBroAnalysisTools.setBound(bound);
			pipeBroAnalysisTools.getBadSwitchs().removeAll(pipeBroAnalysisTools.getBadSwitchs());
