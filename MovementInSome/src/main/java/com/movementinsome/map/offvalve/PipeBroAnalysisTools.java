...
import com.movementinsome.map.offvalve.asynctask.PipeBroAnalysis;
import com.movementinsome.map.offvalve.asynctask.PipeBrokenAnalysisUpload;
...
public class PipeBroAnalysisTools {
	...
	public PipeBroAnalysisTools(Context context, MyMapView map) {
	...
	public void analysisGraphicPoint(final float x, final float y,final String isTrue){
		...
		if(analysisTv == null){
			analysisTv = new TextView(context);
			analysisTv.setOnClickListener(new OnClickListener() {
				@Override
		...
		//修正前的点渲染
		if("0".equals(isTrue)){
			analysisTv.setText("爆管分析点\n(单击修正)");
			symbol = new PictureMarkerSymbol(
					context.getResources().getDrawable(R.drawable.dot3));
			//修正后的点渲染
		}else if ("1".equals(isTrue)){
			analysisTv.setText("修正后位置\n(单击分析)");
			symbol = new PictureMarkerSymbol(
					context.getResources().getDrawable(R.drawable.dot1));
		}

		map.getCallout().show(analysispoint,analysisTv);
		grapLayer.removeAll();
	...
	private void reviseDialog(){
		...
		.setNegativeButton("修正", new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				analysisType = "-1";
				PipeBrokenAnalysisUpload analysisUpload = 
					new PipeBrokenAnalysisUpload(PipeBroAnalysisTools.this);
	...
	public void showFindPoint(float x ,float y){
		...
		try {
			...
			if(ids.length <= 0 ){
				Toast.makeText(context, "请在工作面范围内查找", 4).show();
				return;
			}
	...
	public void showList(){
		if (data != null&&data.size()>0) {
			...
			PipeBroAnalysisTools.this.setActivityName("analysisFind");
			// PipeBroAnalysisTools.this.setPipeBreakenData("");
			...
			for (Map<String, String> p : data) {
				...
				String type = null==p.get("MustBeClosed")?String.valueOf("false"):p.get("MustBeClosed");
				...
				if (ISAbnormal.equals("true")) {// 先判断阀门是否异常
					symbol = new PictureMarkerSymbol(context.getResources()
							.getDrawable(R.drawable.valve_damage));
				} else {// 判断阀门是否必须关闭
					if (type == null) {
						type = "true";
					}
					if (type.endsWith("true")) {
						symbol = new PictureMarkerSymbol(context
								.getResources()
								.getDrawable(R.drawable.valve_close));
					} else {
						symbol = new PictureMarkerSymbol(context
								.getResources()
								.getDrawable(R.drawable.valve_close_n));
					}
				}
				grapLayer.addGraphic(new Graphic(point, symbol, attr, i++));
			}
			symbol = new PictureMarkerSymbol(context.getResources()
					.getDrawable(R.drawable.pipe_break_point));
				...
				public void onClick(View arg0) {
					...
					if(badSwitchs.size()>0){
						...
						try {
							...
							PipeBroAnalysis analysisTask = new PipeBroAnalysis(PipeBroAnalysisTools.this, true,analysispoint);
	...
	private class BgfxCursorAdapter extends BaseAdapter{
		...
		public View getView(final int position, View convertView, ViewGroup parent) {
			...
			ptlistClose.setVisibility(View.VISIBLE);ptlistClose.setOnClickListener(new OnClickListener() {
			...
			String ISAbnormal = "";
			try {
				ISAbnormal = null==data.get(position).get("ISAbnormal")?"":data.get(position).get("ISAbnormal");
			} catch (Exception e) {
				// TODO: handle exception
			}
			String mustClose = "";
			try {
				mustClose = null==data.get(position).get("MustBeClosed")?"":data.get(position).get("MustBeClosed");
			} catch (Exception e) {
				// TODO: handle exception
			}
			String Ord = "";
			try {
				Ord = null==data.get(position).get("Ord")?"":data.get(position).get("Ord");
			} catch (Exception e) {
				// TODO: handle exception
			}
			String state = null==data.get(position).get("state")?"":data.get(position).get("state");
			if(state.equals("true")){
	...
	private void showFac(int arg2){
		...
		float x = Float.parseFloat(points.get(0));
		float y = Float.parseFloat(points.get(1));*/
		PipeBroAnalysisTools.this.showFindPoint2(data.get(arg2));
	}
