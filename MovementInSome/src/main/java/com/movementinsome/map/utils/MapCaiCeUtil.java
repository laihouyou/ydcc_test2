...
public class MapCaiCeUtil {
	...
	public MapCaiCeUtil(MyMapView map, Context context) {
		...
		view= LayoutInflater.from(context).inflate(R.layout.caice_item,null);
...
//				tv.setText(getAreaString());
				...
				TextView baocun= (TextView) view.findViewById(R.id.baocun);
				TextView tiandan= (TextView) view.findViewById(R.id.tiandan);
				TextView chexiao= (TextView) view.findViewById(R.id.chexiao);
				TextView qingchu= (TextView) view.findViewById(R.id.qingchu);
				baocun.setOnClickListener(new View.OnClickListener() {
					@Override
					public void onClick(View v) {
						Toast.makeText(context,"已保存，可继续采集",Toast.LENGTH_SHORT).show();
					}
				});
				tiandan.setOnClickListener(new View.OnClickListener() {
					@Override
					public void onClick(View v) {
						...
						newFormInfo.putExtra("template","ins_hidden_trouble_form_zs.xml");//暂时先打开问题上报单
						context.startActivity(newFormInfo);
						map.getCallout().hide();
						grapPayer.removeAll();
						grapLayer.removeAll();
						ptPrevious=null;//防止填单回来后点绘制点还存在上一个点。
					}
				});
				chexiao.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View v) {
						...
						if(size >=1) {
							...
							map.getCallout().hide();
							map.getCallout().show(ptPrevious,view);
						}
					}
				});
...
//				map.getCallout().hide();
				...
				tv.setOnClickListener(new View.OnClickListener() {
					@Override
					public void onClick(View v) {
						Toast.makeText(AppContext.getInstance(),"已完成采点"+"\n"+listPoint.toString(),Toast.LENGTH_SHORT).show();
						grapPayer.removeAll();
						map.getCallout().hide();
						Intent newFormInfo = new Intent(context,RunForm.class);
						HashMap<String, String> params = new HashMap<String, String>();
						params.put("listPoint",listPoint.toString());
						newFormInfo.putExtra("iParams", params);
						newFormInfo.putExtra("template","ins_hidden_trouble_form_zs.xml");//暂时先打开问题上报单
						context.startActivity(newFormInfo);
					}
