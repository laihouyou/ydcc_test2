...
public class ZsPollingOperater  extends DataOperater implements IDataOperater {

	public ZsPollingOperater(Context context,
		...
		if(nearObject!=null&&valveList!=null){
			...
			if(insPatrolAreaDataList!=null){
				for(int i =0 ;i<insPatrolAreaDataList.size();++i){
					if(nearObject.getObjId().equals(insPatrolAreaDataList.get(i).getId()+"")){
						insPatrolAreaDatas.add(insPatrolAreaDataList.get(i));
					}
				}
			}
		}
		List<Map<String, Object>> data = getListData(valveNearDataCache, "InsPatrolDataVO");
