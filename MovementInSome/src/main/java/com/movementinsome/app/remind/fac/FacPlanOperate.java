...
public class FacPlanOperate extends PlanOperate implements IPlanOperate {
	...
	static Map<String, FacTranceTask> inTranTaskCache = new HashMap<String, FacTranceTask>(); // 启动执行路段判断线程
	// 待巡路段缓存
	...
	public void insBeginDo(Context context, String doId, String workTaskNum) {
		...
		if (!inTranTaskCache.containsKey(doId)) {
			...
			FacTranceTask workTran = new FacTranceTask(this, context, doId);
			inTranTaskCache.put(doId, workTran);
			workTran.start(0, 5000);
		} else {
			FacTranceTask workTran = inTranTaskCache.get(doId);
			workTran.cancel();
			workTran.start(0, 5000);
		}
	}
