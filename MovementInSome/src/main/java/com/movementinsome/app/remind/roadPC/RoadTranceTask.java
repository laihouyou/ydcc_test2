...
public class RoadTranceTask extends TimerTask {
	...
	protected PcRoadPlanOperate planOper;
	...
	public RoadTranceTask(PcRoadPlanOperate planOper, Context context, String doId) {
	...
	protected void updateObjState(InsPatrolOnsiteRecordExtVO nearObj) {
		...
		try{
			upLocState = true;
			InsCheckFacRoad patrolDataRemove = null;
			List list =planOper.getWaitForDoDataCache(doId);
	
			//for (InsPatrolDataVO patrolData : planOper.getWaitForDoDataCache(doId)) {
			Iterator<InsCheckFacRoad> iterator = list.iterator();
	        while(iterator.hasNext()){
	        	
	        	InsCheckFacRoad patrolData = iterator.next();
				if (patrolData.getGuid().equals(nearObj.getId())) {
					...
					try {
						//判断巡查周期及巡查次数需不需要初始化
						if(patrolData.getShapeStr()!=null&&nearObj.getPatrolStartDate()!=null){
							if (!patrolData.getShapeStr().subSequence(0, 10).equals(nearObj.getPatrolStartDate().subSequence(0, 10))){
...
//								}
						...
						patrolData.setShapeStr(nearObj.getPatrolStartDate());
...
//							}
						...
						planOper.InsCheckFacRoadDao.update(patrolData); // 理新路段的巡查时间和巡查次数
...
//								patrolData.setInsCount(Long.valueOf(-1));
								planOper.InsCheckFacRoadDao.update(patrolData);
							}else{
	...
	private void removeWaitForDoDataCache(String id){
		for (InsCheckFacRoad patrolData : planOper.getWaitForDoDataCache(doId)) {
