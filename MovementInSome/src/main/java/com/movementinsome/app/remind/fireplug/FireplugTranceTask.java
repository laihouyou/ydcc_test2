...
public class FireplugTranceTask extends TimerTask{
	// true时使用后台进程线程。只要剩下的程序记叙运行，后台进程线程就会执行。
	Timer myTimer;

	private FireplugPlanOperate planOper;
	...
	public FireplugTranceTask(FireplugPlanOperate planOper, Context context,
			String doId) {
	...
	public void run() {
		...
		if ( location != null) {
			...
			noSite = navLocalGeoData.findNearValve(location, planOper.getWaitForDoDataCache(doId));
			if (noSite != null) {
				if (null != noSite.getObjName()) {
					...
					// Toast.makeText(context,
					// "距离签到点："+noQianDD.getObjName()+"还有多少"+String.valueOf(noQianDD.getMinDistance()),Toast.LENGTH_LONG
					// ).show();
				}
			}
		}
