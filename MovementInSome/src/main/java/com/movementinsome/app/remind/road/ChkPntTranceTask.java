...
public class ChkPntTranceTask extends TimerTask {

	// true时使用后台进程线程。只要剩下的程序记叙运行，后台进程线程就会执行。
	Timer myTimer;
	
	private ChkPntPlanOperate planOper;
...
//	private List<InsPatrolDataVO> obj;
	...
	public ChkPntTranceTask(ChkPntPlanOperate planOper,Context context,String doId){
	...
	/*			NearObject noQianDD = NavLocalGeoData.getInstance().findNearQianDaoDian(AppContext.getInstance().getCurLocation(),50);
				...
				noQianDD = navLocalGeoData.findNearQianDaoDian(AppContext.getInstance().getCurLocation(),planOper.getWaitForDoDataCache(doId));
				if (noQianDD != null){
