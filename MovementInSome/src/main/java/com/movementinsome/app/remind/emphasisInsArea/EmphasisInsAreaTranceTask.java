...
public class EmphasisInsAreaTranceTask extends TimerTask{
	// true时使用后台进程线程。只要剩下的程序记叙运行，后台进程线程就会执行。
	Timer myTimer;

	private EmphasisInsAreaOperate planOper;
	...
	public EmphasisInsAreaTranceTask(EmphasisInsAreaOperate planOper, Context context,
			String doId) {
	...
	public void run() {
		...
		if ( location != null) {
			...
			noSite = navLocalGeoData.findNearBsEmphasisInsArea(location, planOper.getWaitForDoDataCache(doId));
			if (noSite != null) {
