...
public class FireplugPlanOperate extends PlanOperate implements IPlanOperate {
	...
	static Map<String, FireplugTranceTask> inTranTaskCache = new HashMap<String, FireplugTranceTask>(); // 启动执行路段判断线程
	// 待巡路段缓存
	...
	public void insBeginDo(Context context, String doId, String workTaskNum) {
		...
		if (!inTranTaskCache.containsKey(doId)) {
			...
			FireplugTranceTask workTran = new FireplugTranceTask(this, context, doId);
			inTranTaskCache.put(doId, workTran);
			workTran.start(0, 5000);
		} else {
			FireplugTranceTask workTran = inTranTaskCache.get(doId);
			workTran.cancel();
			workTran.start(0, 5000);
		}
	}
