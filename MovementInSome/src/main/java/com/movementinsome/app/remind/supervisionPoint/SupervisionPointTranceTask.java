...
public class SupervisionPointTranceTask extends TimerTask {
	// true时使用后台进程线程。只要剩下的程序记叙运行，后台进程线程就会执行。
	Timer myTimer;

	private SupervisionPointPlanOperate planOper;
	...
	public SupervisionPointTranceTask(SupervisionPointPlanOperate planOper, Context context,
			String doId) {
	...
	public void run() {
		...
		if ( location != null) {
			...
			noSite = navLocalGeoData.findNearBsSupervisionPoint(location, planOper.getWaitForDoDataCache(doId));
			if (noSite != null) {
