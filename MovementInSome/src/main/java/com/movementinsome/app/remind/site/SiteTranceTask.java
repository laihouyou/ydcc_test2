...
public class SiteTranceTask extends TimerTask {

	// true时使用后台进程线程。只要剩下的程序记叙运行，后台进程线程就会执行。
	Timer myTimer;
	
	private SitePlanOperate planOper;
...
//	private List<InsPatrolDataVO> obj;
	...
	public SiteTranceTask(SitePlanOperate planOper,Context context,String doId){
...
/*			NearObject noQianDD = NavLocalGeoData.getInstance().findNearQianDaoDian(AppContext.getInstance().getCurLocation(),50);
			...
			noSite = navLocalGeoData.findNearSite(location, planOper.getWaitForDoDataCache(doId));
			if (noSite != null){
