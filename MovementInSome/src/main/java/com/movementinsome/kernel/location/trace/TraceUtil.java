...
public class TraceUtil {
	...
	public static Location callGear(Context ctx, ArrayList<CellInfo> cellIds)
		...
		if (cellIds == null || cellIds.size() == 0) {
			TraceUtil.alert(ctx, "cell request param null");
			return null;
		}
		;

		try {
			result = TraceUtil.getResponseResult(ctx,
			...
			loc.setTime(TraceUtil.getUTCTime());
			return loc;
	...
	public static String getResponseResult(Context ctx, String path,
		...
		InputStream inStream = TraceUtil.sendPostRequest(path,
				getRequestParams(cellInfos), "UTF-8");
		if (inStream != null) {
			byte[] datas = TraceUtil.readInputStream(inStream);
