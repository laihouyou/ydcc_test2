...
public class Gcj022Gps {
	...
	// 84-gcj02
	public static Map<String, Double> wgs2gcj(double wgsLon, double wgsLat) {
		Map<String, Double> localHashMap = new HashMap<String, Double>();
		double lontitude = wgsLon
				+ (((Double) transform(wgsLon, wgsLat).get("lon")).doubleValue() - wgsLon);
		double latitude = (wgsLat + (((Double) (transform(wgsLon, wgsLat)).get("lat"))
				.doubleValue() - wgsLat));
		localHashMap.put("lon", lontitude);
		localHashMap.put("lat", latitude);
		return localHashMap;
