...
public class BaiduMapUtil {
     ...
     * @param distance
     *            距离(单位为米)
     * @return [lng1,lat1, lng2,lat2] 矩形的左上角(lng1,lat1)和右下角(lng2,lat2)
     */
    public static double[] getRectangle_TopLeftAndRightBottom(double lng, double lat, long distance) {
        float delta = 111000;
        if (lng != 0 && lat != 0) {
            double lng1 = lng - distance
                    / Math.abs(Math.cos(Math.toRadians(lat)) * delta);
