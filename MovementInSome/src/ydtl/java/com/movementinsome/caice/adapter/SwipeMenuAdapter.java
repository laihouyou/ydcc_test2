...
public class SwipeMenuAdapter extends ListBaseAdapter<StatisticsChileListVo> {
    ...
    public void onBindItemHolder(SuperViewHolder holder, final int position) {
        final StatisticsChileListVo item=getDataList().get(position);
        ...
        if (item.getIsLeak()!=null){
            if (item.getIsLeak().equals("否")){
                gather_type_fac.setText("探漏类型:非漏点");
            }else  if (item.getIsLeak().equals("是")){
                gather_type_fac.setText("探漏类型:确认漏点");
            }else {
                gather_type_fac.setText("探漏类型:疑似漏点");
            }
        }
        resubmit_date_tv_fac.setText("时间:"+item.getUploadTime());
        adder_tv_fac.setText("地址:"+item.getHappenAddr());

        //设置图标
        if (item.getDateType().equals(MapMeterMoveScope.POINT)){
            projcet_type_fac.setBackgroundResource(R.drawable.point_left);
        }else if (item.getDateType().equals(MapMeterMoveScope.LINE)){
            projcet_type_fac.setBackgroundResource(R.drawable.line_left);

        }

        if (item.getIsPresent()!=null){
            if (item.getIsPresent().equals("0")){
            ...
            }else if (item.getIsPresent().equals("1")){
        ...
        location_text_fac.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                intent.putExtra("facName", item.getFacName());
                intent.putExtra("dateType", item.getDateType());
